...
| `rag-backend-cross-encoder` | Pairwise `rerank` (`:score-fn` / `:batch-fn`) | [`egao1980/rag-backend-cross-encoder`](https://github.com/egao1980/rag-backend-cross-encoder) |
...
Default `rerank` is identity (score desc). Cross-encoder is `rag-backend-cross-encoder` (`:score-fn` / `:batch-fn`; default token overlap is not a CE model). Default ingest chunker is passthrough unless you bind a `rag-chunker`. Optional `:sparse-encoder` on the pipeline fills `rag-chunk-sparse` / `rag-query-sparse`.
